...
int be_lastsession(const struct proxy *be)
{
	if (be->be_counters.last_sess)
		return clock_sec(now) - be->be_counters.last_sess;
...
void back_try_conn_req(struct stream *s)
{
	...
	if (sc->state == SC_ST_ASS) {
		...
		if (conn_err == SF_ERR_INTERNAL) {
			...
			s->logs.t_queue = tv_ms_elapsed(&s->logs.tv_accept, &CLOCK_TO_TV(now));
...
void back_handle_st_rdy(struct stream *s)
{
	...
	if (unlikely(obj_type(s->target) == OBJ_TYPE_APPLET)) {
		...
		if (tv_iszero(&s->logs.tv_request))
			s->logs.tv_request = CLOCK_TO_TV(now);
		s->logs.t_queue   = tv_ms_elapsed(&s->logs.tv_accept, &CLOCK_TO_TV(now));
		be_set_sess_last(s->be);
	}
...
int be_downtime(struct proxy *px) {
	if (px->lbprm.tot_weight && px->last_change < clock_sec(now))  // ignore negative time
		return px->down_time;

	return clock_sec(now) - px->last_change + px->down_time;
}
