			   ...
			   char *frame, size_t size)
{
	...
	*ctx = NULL;
	...
	SPOE_PRINTF(stderr, "%d.%06d [SPOE/%-15s] %s: appctx=%p"
		    ...
		    (int)clock_sec(now), (int)clock_usec(now), agent->id,
		    __FUNCTION__, appctx,
...
spoe_handle_disconnecting_appctx(struct appctx *appctx)
{
	...
	switch (ret) {
		case -1: /* error  */
			SPOE_PRINTF(stderr, "%d.%06d [SPOE/%-15s] %s: appctx=%p"
				    " - error on frame (%s)\n",
				    (int)clock_sec(now), (int)clock_usec(now),
...
spoe_queue_context(struct spoe_context *ctx)
{
	...
	SPOE_PRINTF(stderr, "%d.%06d [SPOE/%-15s] %s: stream=%p"
		    ...
		    (int)clock_sec(now), (int)clock_usec(now), agent->id, __FUNCTION__,
		    ctx->strm);
...
spoe_init(struct proxy *px, struct flt_conf *fconf)
{
	 ...
	 * parsing. Finish initialization. */
        conf->agent_fe.last_change = clock_sec(now);
