...
{
	if ((s->cur_state != SRV_ST_STOPPED) || s->last_change >= clock_sec(now))		// ignore negative time
		return s->down_time;

	return clock_sec(now) - s->last_change + s->down_time;
}

int srv_lastsession(const struct server *s)
{
	if (s->counters.last_sess)
		return clock_sec(now) - s->counters.last_sess;
...
void server_recalc_eweight(struct server *sv, int must_update)
{
	...
	if (clock_sec(now) < sv->last_change || clock_sec(now) >= sv->last_change + sv->slowstart) {
	...
	if ((sv->next_state == SRV_ST_STARTING) && (px->lbprm.algo & BE_LB_PROP_DYN))
		w = (px->lbprm.wdiv * (clock_sec(now) - sv->last_change) + sv->slowstart) / sv->slowstart;
	else
		w = px->lbprm.wdiv;
	...
	if (srv->slowstart) {
		...
		if (srv->next_state == SRV_ST_STARTING) {
			task_schedule(srv->warmup,
			              tick_add(now_ms,
			                       MS_TO_TICKS(MAX(1000, (clock_sec(now) - srv->last_change)) / 20)));
		}
	}
...
static void srv_update_status(struct server *s, int type, int cause)
{
	...
	if (srv_prev_state != s->cur_state) {
		if (srv_prev_state == SRV_ST_STOPPED) {
			...
			if (s->last_change < clock_sec(now))                        // ignore negative times
				s->down_time += clock_sec(now) - s->last_change;
		}
		else if (s->cur_state == SRV_ST_STOPPED) {
			/* server was up and is currently down */
			s->counters.down_trans++;
		}
		s->last_change = clock_sec(now);
	}
	...
	if (prev_srv_count && s->proxy->srv_bck == 0 && s->proxy->srv_act == 0)
		 ...
		 */
		if (s->proxy->last_change < clock_sec(now))         // ignore negative times
			s->proxy->down_time += clock_sec(now) - s->proxy->last_change;
		s->proxy->last_change = clock_sec(now);
	}
}
